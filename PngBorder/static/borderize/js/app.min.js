...
const kernelSize = 5;
let textModeEnabled = !1,
    currentUpdateTimeout;

function init() {
    borderProps = new BorderProperties(4, 2, [255, 255, 255], [0, 0, 0], 170, .2), textCanvas = new CanvasText("", 120, "comic-sans", "#000000", "center", .5), webGLCanvas = new CanvasWebGL("canvas"), imageCanvas = new CanvasImage;
    let fontSelect = document.getElementById("font");
    fonts.forEach(font => {
        let option = document.createElement("option");
        option.value = font, option.text = font, option.style = "font-family: " + font + ";", fontSelect.appendChild(option)
    }), textInput = document.getElementById("font-input"), WebFont.load({
        google: {
            families: fonts
        },
        active: function() {
            textCanvas.text = "Loaded", font = fonts[0], textInput.value = textCanvas.text, textInput.disabled = !1, draw(), console.log("All loaded")
        },
        loading: function() {
            textCanvas.text = "Loading Fonts", textInput.value = textCanvas.text, textInput.disabled = !0, draw(), console.log("The fonts are loading")
        }
...
function draw() {
    let imageWidth;
    document.getElementById("download").disabled = !0, textModeEnabled ? (textCanvas.drawText(borderProps.extraSize, 5), webGLCanvas.draw(textCanvas.canvas, borderProps)) : (imageCanvas.drawImage(borderProps.extraSize, 5), webGLCanvas.draw(imageCanvas.canvas, borderProps)), document.getElementById("download").disabled = !1, document.getElementById("canvas").style.width = "100%"
}
...
function onInputChange(name, value) {
    ...
    if ("lineSeparation" == name) {
        ...
        let oldValue = borderProps.borderColor,
            currentValue;
        ...
        let oldValue = borderProps.shadowColor,
            currentValue;
    ...
    } else if ("appMode" == name) changeAppMode(value);
...
function update() {
    clearTimeout(currentUpdateTimeout), currentUpdateTimeout = setTimeout(() => {
        draw()
    }, waitToUpdate)
}
...
function downloadImageDetail() {
    ...
    link.download = title + ".png", link.href = imageURL, link.click(), image.disabled = !0
}

function download() {
    let image = document.getElementById("canvas").toDataURL();
    var link = document.createElement("a");
    link.download = "my-image.png", link.href = image, link.click(), document.getElementById("download").disabled = !0
}
...
function changeAppMode(mode) {
    ...
    "text" === mode ? (textControls.hidden = !1, imageControls.hidden = !0, textModeEnabled = !0) : (textControls.hidden = !0, imageControls.hidden = !1, textModeEnabled = !1), draw()
}
    ...
    if (file.type.match("image.*")) {
        ...
        reader.onload = event => {
            if (event.target.readyState == FileReader.DONE) {
                let image = new Image;
                image.onload = () => {
                    imageCanvas.loadImage(image), draw()
